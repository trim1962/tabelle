...
}
{
	...
	\node (aa) at   ({cos(\x/2} , {sin(\x/2} ) [label=above right:$\arco$] {};
	\draw(0,0) -- ({\raggio*cos(\x} , {\raggio*sin(\x})node[midway,left ]{a} node [above]{P};
	\draw[dashed] ({\raggio*cos(\x} , 0 ) -- ({\raggio*cos(\x} , {\raggio*sin(\x} )node[midway,left]{c}  ;
	\draw [dashed](0, {\raggio*sin(\x} ) -- ({\raggio*cos(\x} , {\raggio*sin(\x} );
	\draw[dashed] (0 , 0 ) -- (0 , {\raggio*sin(\x} )node [above left]{K};
	\draw [dashed](0, 0 ) -- ({\raggio*cos(\x} , 0 )node[midway,above]{b} node [below]{H};;
}
