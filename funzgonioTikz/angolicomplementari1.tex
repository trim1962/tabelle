...
\begin{document}
	...
	\draw(Q)--(QY) node[midway,above] {$\cos(\ang{90}-\alpha)$};
	\draw(P)--(PX) node[midway,right] {$\sen(\alpha)$};
	\draw(Q)--(QX)node[midway,right] {$\sen(\ang{90}-\alpha)$};
	\draw [->] (0:\sraggio/3) arc (0:\sangolo:\sraggio/3) node[below] {$\ang{90}-\alpha$};
